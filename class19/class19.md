...
- [Examine IgG Ab titer level](#examine-igg-ab-titer-level)
...
Add some major milestone timepoints to our plots:
...
The switch to the aP (newer acellular formalization) lead to a rise in
Pertussis after a lag, potentially be due to the anti-vaccine trend,
lower coverage of the new vaccine, evolution of the bacteria, etc.

The 2020 lockdown and social distancing measures mitigated the spread of
all diseases, especially those respiratory diseases, leading to a
decline in the trend again.
...
They make our data available via JSON format API endpoints - basically
the database tables in a key:value type format,
i.e. {“infancy_vac”:“wP”}.

To read this, we can use the `read_json()` function from the
**jsonlite** package.
...
largest sample of its type ever assembled.

``` r
specimen <- read_json(path = "http://cmi-pb.org/api/v5_1/specimen", simplifyVector = TRUE)
ab_titer <- read_json(path = "http://cmi-pb.org/api/v5_1/plasma_ab_titer", simplifyVector = TRUE)
```
...
We need to join or link these tables with the `subjext` table so that we
...
Now lets join the `ab_titer` table with our `meta` table so we can have
all the information about a given Ab measurement.
...
More advances analysis digging into individual antigen responses over
time:
...
There are lots of coolthigns to explore in this dataset and we need
